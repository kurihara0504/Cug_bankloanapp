...
/**
 * 帳票情報に紐づく外貨定期預金預入のご案内のリスト内部
 *
 *　業務API「外貨定期預金預入案内照会」からのレスポンス
 */
...
public class ReportForeignCurrencyDepositOpenDto {
  ...
  /**
   * 外貨定期預金預入のご案内識別子を取得する.
   *
   * 外貨定期預金預入のご案内識別子を呼び出し元に返却する
   * @return 外貨定期預金預入のご案内識別子
   */
  public String getId() {
    return id;
  }
  /**
   * 外貨定期預金預入のご案内識別子を設定する.
   *
   * 受け取った外貨定期預金預入のご案内識別子を設定する
   * @param id 外貨定期預金預入のご案内識別子
   */
