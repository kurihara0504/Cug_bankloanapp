...
/**
 * 業務API「ご返済のご案内」呼び出しクラス.
 *
 * 業務API「ご返済のご案内」を呼び出すクラス
 */
@Component
public class InformationRepaymentLogic {
  /** 銀行サービス基盤API.*/
  @Autowired
   ...
   * 業務API「ご返済のご案内」を呼出す.
   *
   * 業務API「ご返済のご案内」のパラメータを構築し、API呼出しを行う。呼出し結果をDtoで返却する
   * @param userId ユーザ識別子
   * @param accountId 口座識別子
   * @param reportId ご返済のご案内識別子
   * @return 業務API「ご返済のご案内」呼出し結果
   * @throw BaseRestAppStatusException HTTPレスポンスステータスが404で返却された場合にスローする
   */
  public InformationRepaymentDto callInformationRepaymentApi(String userId ,String accountId, String reportId) {
    // パスを生成する
    ...
    requestPath.append(bankBookConstantValues.findByKeyName(CugBankBookConstants.INFORMATION_REPAYMENT_API_PATH));
    ...
    return bankService.request(
        bankBookConstantValues.findByKeyName(CugBankBookConstants.INFORMATION_REPAYMENT_API_CODE),
        HttpMethod.GET,
        userId,
        requestPath.toString())
          .registAppException(status400)
          .registAppException(status404)
          .done(InformationRepaymentDto.class);
  }
